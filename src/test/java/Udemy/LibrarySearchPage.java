...
public class LibrarySearchPage extends TestUtils {
    @Test
    public void librarySearchHappyPathValidNameTest() {
    ...
    @Test
    public void librarySearchHappyPathRandomNameTest() {
        WebDriverManager.chromedriver().setup();
        WebDriver driver = new ChromeDriver();
        ...
        driver.findElement(By.xpath("//input[contains(@value,'PUB')]")).click();
        driver.findElement(By.xpath("//input[contains(@type, 'text')]")).sendKeys("jhfksjdh");
        driver.findElement(By.xpath("//button[contains(@class,'search-btn')]")).click();
        driver.manage().timeouts().implicitlyWait(1, TimeUnit.SECONDS);

        WebElement result = driver.findElement(By.xpath("//div[@class='header-message']"));
        Assert.assertFalse(result.getText().toLowerCase().contains("we found"));
        driver.quit();
    }
}
